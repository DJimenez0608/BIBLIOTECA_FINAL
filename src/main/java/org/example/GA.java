...
public class GA {
    ...
    private static final int PUERTOGA2 = 5570;
    private static final String RUTA_DB = "C:\\Users\\dicaj\\Desktop\\ULTIMO INTENTO\\Biblioteca_distribuidos-842126ed737b86d6648b39a2daad0ea05c1cb1bf\\DB.txt";
    private static final String RUTA_PRESTAMOS = "C:\\Users\\dicaj\\Desktop\\ULTIMO INTENTO\\Biblioteca_distribuidos-842126ed737b86d6648b39a2daad0ea05c1cb1bf\\Prestamos.txt";
    ...
    public void iniciar() {
        ...
        System.out.println(" GA conectado a GA2 en puerto " + PUERTOGA2 + " (DEALER)...");
    ...
    private void inicializarDealerSocket() {
        ...
        dealer.connect("tcp://localhost:" + PUERTOGA2);
        System.out.println(" Socket DEALER conectado a GA2 en localhost:" + PUERTOGA2);
    ...
    private String procesarSolicitud(String solicitud) {
        ...
        // Verificar si GA2 procesó exitosamente
        if (respuestaGA2 == null || respuestaGA2.startsWith("Error")) {
            System.out.println("GA:  GA2 retornó error, NO procesando localmente: " + respuestaGA2);
            return respuestaGA2;
        }
        
        // Si GA2 fue exitoso, sincronizar procesando localmente
        if (solicitud.startsWith("Disponibilidad?")) {
            // Verificar si la respuesta de GA2 indica éxito para préstamo
            if (respuestaGA2.equals("SI")) {
                System.out.println("GA:  GA2 procesó exitosamente, sincronizando localmente (préstamo)...");
                String resultado = manejarDisponibilidad(solicitud); // Procesar localmente (solo para sincronización)
            ...
            // Verificar si la respuesta de GA2 indica éxito para devolución
            if (respuestaGA2.equals("Devolución registrada exitosamente")) {
                ...
                manejarDevolucion(solicitud); // Procesar localmente (solo para sincronización)
            } else {
                System.out.println("GA:  GA2 retornó '" + respuestaGA2 + "', NO procesando localmente");
            }
            
        } else if (solicitud.startsWith("RENOVAR")) {
            // Verificar si la respuesta de GA2 indica éxito para renovación
            if (respuestaGA2.contains("exitoso") || respuestaGA2.contains("nuevo préstamo")) {
                if (!respuestaGA2.contains("No se pueden hacer más renovaciones") && !respuestaGA2.startsWith("Error")) {
                    ...
                    manejarRenovacion(solicitud); // Procesar localmente (solo para sincronización)
                } else {
                    System.out.println("GA:  GA2 retornó '" + respuestaGA2 + "', NO procesando localmente");
                }
            } else {
    ...
    private String manejarDisponibilidad(String solicitud) {
        ...
        if (estado.equals("DISPONIBLE")) {
            ...
            if (actualizado) {
                ...
                System.out.println("GA:  Llamando a registrarPrestamo() para libro ID: " + idLibro);
    ...
    private List<String> leerDB() {
        try {
            ...
            if (!archivo.exists()) {
                System.err.println("GA:  Error - Archivo DB.txt no encontrado en: " + RUTA_DB);
                return null;
            }
    ...
    private void registrarPrestamo(String idLibro) {
        try {
            ...
            System.out.println("GA:  Ruta de Prestamos.txt: " + RUTA_PRESTAMOS);
